...
const demographicData = { total: 4397073, male: 2192452, female: 2204376, area: 696, source: "KNBS 2019 KPHC · Volume I" };
...
function NetworkMap() {
  ...
  return <Panel className={`map-panel map-style-${mapStyle}`}>
    ...
    <div className="map-style-switcher" role="group" aria-label="Map style"><span>MAP STYLE</span>{(["dark", "street", "satellite"] as MapStyle[]).map((style) => <button key={style} className={mapStyle === style ? "active" : ""} onClick={() => setMapStyle(style)}>{style}</button>)}</div>
    {showLayers && <div className="layer-control-panel"><div className="layer-control-head"><div><strong>Map layers</strong><small>Toggle simulated digital-twin overlays</small></div><X size={14} onClick={() => setShowLayers(false)} /></div><div className="layer-group"><span>County context</span><button className={layers.dmas ? "on" : ""} onClick={() => toggleLayer("dmas")}>Nairobi County</button><button className={layers.reservoirs ? "on" : ""} onClick={() => toggleLayer("reservoirs")}>Water facilities</button></div><div className="layer-group"><span>Monitoring</span><button className={layers.sensors ? "on" : ""} onClick={() => toggleLayer("sensors")}>Pressure sensors</button><button className={layers.alerts ? "on" : ""} onClick={() => toggleLayer("alerts")}>Leak alerts</button></div><div className="layer-group"><span>AI overlays</span><button className={layers.ai ? "on" : ""} onClick={() => toggleLayer("ai")}>Leak probability</button><button className={layers.dmas ? "on" : ""} onClick={() => toggleLayer("dmas")}>DMAs</button></div></div>}<div className="map-stage"><svg viewBox="0 0 860 380" className="network-svg" aria-label="Nairobi County verified boundary and high-level water alerts" onPointerDown={(e) => { const startX = e.clientX; const startY = e.clientY; const sx = offset.x; const sy = offset.y; const move = (ev: PointerEvent) => setOffset({ x: sx + ev.clientX - startX, y: sy + ev.clientY - startY }); const up = () => { window.removeEventListener("pointermove", move); window.removeEventListener("pointerup", up); }; window.addEventListener("pointermove", move); window.addEventListener("pointerup", up); }}>
      ...
      <div className="county-stats-panel"><div className="county-stats-heading"><div><span className="eyebrow">COUNTY SNAPSHOT</span><h3>Nairobi County</h3></div><span className="county-status">2019 CENSUS</span></div><div className="county-stats-grid"><span><b>4.40M</b><small>Residents</small></span><span><b>696 km²</b><small>Area</small></span><span><b>23</b><small>Active alerts</small></span><span><b>42.6%</b><small>NRW today</small></span></div><div className="demographic-chart"><div className="demographic-chart-heading"><span>Population by sex</span><small>Enumerated residents</small></div><div className="sex-bars"><div><span>Female <b>{((demographicData.female / demographicData.total) * 100).toFixed(1)}%</b></span><i className="female-bar" style={{ width: `${(demographicData.female / demographicData.total) * 100}%` }} /></div><div><span>Male <b>{((demographicData.male / demographicData.total) * 100).toFixed(1)}%</b></span><i className="male-bar" style={{ width: `${(demographicData.male / demographicData.total) * 100}%` }} /></div></div></div><div className="demographic-chart population-chart"><div className="demographic-chart-heading"><span>Population baseline</span><small>{demographicData.total.toLocaleString()} residents</small></div><div className="population-bar"><i style={{ width: "100%" }} /></div><div className="population-labels"><span>2019 KNBS census</span><b>4,397,073</b></div></div><div className="county-stats-footer"><span><i className="status-green" />Boundary verified</span><button onClick={fitCounty}>Fit county <LocateFixed size={12} /></button></div><small className="county-source">Source: {demographicData.source}</small></div>
