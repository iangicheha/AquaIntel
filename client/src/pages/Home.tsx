...
const pipeMetadata: Record<string, { diameter: string; material: string; status: string; pressure: string; flow: string; upstream: string; downstream: string; sensors: string; maintenance: string; risk: string }> = {
  "DP-6B-143": { diameter: "200 mm", material: "Ductile iron", status: "Suspected leak", pressure: "31.4 m", flow: "+16%", upstream: "EMB-VAL-06", downstream: "DMA-EMB-06", sensors: "PS-26 · FM-26 · PS-27", maintenance: "Valve inspection · 18 Jun 2026", risk: "88%" },
  "TM-01": { diameter: "900 mm", material: "Steel", status: "Operational", pressure: "48.7 m", flow: "82.1 ML/day", upstream: "KAB-TP-01", downstream: "CBD Junction", sensors: "PS-01 · FM-01", maintenance: "Corrosion survey · 03 Jul 2026", risk: "12%" },
  "DP-03-018": { diameter: "150 mm", material: "PVC", status: "Monitor", pressure: "36.2 m", flow: "8.4 ML/day", upstream: "CBD-VAL-03", downstream: "DMA-CBD-03", sensors: "PS-18 · FM-18", maintenance: "Leak survey · 29 Jul 2026", risk: "74%" },
};
...
function NetworkMap() {
  ...
  const selectedPipe = pipeMetadata[selected] || pipeMetadata["DP-6B-143"];
  ...
  return <Panel className="map-panel">
    ...
    {showLayers && <div className="layer-control-panel"><div className="layer-control-head"><div><strong>Map layers</strong><small>Toggle simulated digital-twin overlays</small></div><X size={14} onClick={() => setShowLayers(false)} /></div><div className="layer-group"><span>Infrastructure</span><button className={layers.pipes ? "on" : ""} onClick={() => toggleLayer("pipes")}>Pipes</button><button className={layers.valves ? "on" : ""} onClick={() => toggleLayer("valves")}>Valves</button><button className={layers.reservoirs ? "on" : ""} onClick={() => toggleLayer("reservoirs")}>Reservoirs</button></div><div className="layer-group"><span>Monitoring</span><button className={layers.sensors ? "on" : ""} onClick={() => toggleLayer("sensors")}>Pressure sensors</button><button className={layers.alerts ? "on" : ""} onClick={() => toggleLayer("alerts")}>Leak alerts</button></div><div className="layer-group"><span>AI overlays</span><button className={layers.ai ? "on" : ""} onClick={() => toggleLayer("ai")}>Leak probability</button><button className={layers.dmas ? "on" : ""} onClick={() => toggleLayer("dmas")}>DMAs</button></div></div>}<div className="map-stage"><svg viewBox="0 0 860 380" className="network-svg" aria-label="Interactive Nairobi County hydraulic distribution network overlay" onPointerDown={(e) => { const startX = e.clientX; const startY = e.clientY; const sx = offset.x; const sy = offset.y; const move = (ev: PointerEvent) => setOffset({ x: sx + ev.clientX - startX, y: sy + ev.clientY - startY }); const up = () => { window.removeEventListener("pointermove", move); window.removeEventListener("pointerup", up); }; window.addEventListener("pointermove", move); window.addEventListener("pointerup", up); }}>
        ...
        <g className="county-context"><path className="county-boundary" d="M72 70 L166 38 L274 48 L366 30 L488 50 L604 35 L742 68 L812 132 L786 214 L816 286 L754 344 L638 356 L534 340 L428 366 L308 344 L204 360 L104 316 L48 238 L66 156 Z" /><path className="county-subdivision" d="M166 38 L204 154 L104 316 M274 48 L320 170 L308 344 M366 30 L430 150 L428 366 M488 50 L520 176 L534 340 M604 35 L626 164 L638 356 M742 68 L700 176 L754 344 M66 156 L320 170 L626 164 L786 214 M48 238 L308 244 L638 248 L816 286" /><text x="344" y="202" className="county-label">NAIROBI COUNTY · LIVE DISTRIBUTION EXTENT</text></g><g className="infra-sites"><g className="site-marker treatment" transform="translate(132 92)" onClick={() => toast("Kabete Water Treatment Plant · 82% output")}><rect x="-10" y="-10" width="20" height="20" rx="3" /><path d="M0 -6 L5 4 H-5 Z" /></g><g className="site-marker reservoir" transform="translate(430 54)" onClick={() => toast("Gigiri Reservoir · 68% full")}><ellipse cy="6" rx="12" ry="4" /><path d="M-12 -5 V6 M12 -5 V6 M-12 -5 C-5 -10 5 -10 12 -5" /></g><g className="site-marker treatment" transform="translate(752 196)" onClick={() => toast("Kipeto Treatment Plant · online")}><rect x="-10" y="-10" width="20" height="20" rx="3" /><path d="M0 -6 L5 4 H-5 Z" /></g><g className="site-marker reservoir" transform="translate(594 334)" onClick={() => toast("Embakasi Reservoir · 74% full")}><ellipse cy="6" rx="12" ry="4" /><path d="M-12 -5 V6 M12 -5 V6 M-12 -5 C-5 -10 5 -10 12 -5" /></g></g>
        <g transform={`translate(${offset.x} ${offset.y}) scale(${zoom})`} className="topology-layer">
          {layers.pipes && <g className="network-pipes">{pipes.map((pipe) => <path key={pipe.id} d={pipe.d} className={pipeClass(pipe)} onClick={() => { setSelected(pipe.id); setShowPipeDetails(true); }}><title>{pipe.id} · {pipe.type} · click to inspect</title></path>)}</g>}<g className="flow-arrows"><path d="M70 92 H180" markerEnd="url(#flowArrow)" /><path d="M250 92 H370" markerEnd="url(#flowArrow)" /><path d="M450 92 H570" markerEnd="url(#flowArrow)" /><path d="M530 166 H650" markerEnd="url(#flowArrow)" /><path d="M430 122 V240" markerEnd="url(#flowArrow)" /></g><g className="flow-particles">{pipes.slice(0, 7).map((pipe, i) => <circle key={`flow-${pipe.id}`} r={i % 2 ? 2.2 : 2.8} className={`flow-particle ${pipe.leak ? "danger" : ""}`}><animateMotion dur={`${3.2 + i * .35}s`} repeatCount="indefinite" path={pipe.d} /></circle>)}</g>
          {(layers.valves || layers.sensors || layers.reservoirs) && <g className="network-nodes">{nodes.map(([x, y, kind], i) => kind === "valve" && layers.valves ? <rect key={i} x={x - 4} y={y - 4} width="8" height="8" className="valve-node" onClick={() => toast(`Valve ${i + 1} selected`)} /> : kind === "reservoir" && layers.reservoirs ? <g key={i} onClick={() => toast("Demo reservoir selected")}><rect x={x - 10} y={y - 7} width="20" height="14" rx="2" className="reservoir-node" /><path d={`M${x - 7} ${y - 2}h14`} className="reservoir-water" /></g> : kind === "sensor" && layers.sensors ? <circle key={i} cx={x} cy={y} r="3" className={i % 7 === 0 ? "sensor-node warning" : "sensor-node"} onClick={() => toast(`Pressure sensor PS-${i + 1} selected`)} /> : null)}</g>}
          ...
          {layers.alerts && <g className="map-alert red live-alert-marker" transform="translate(590 246)" onClick={() => setSelected("DP-6B-143")}><circle r="12" /><circle r="5" /><path d="M0 -17 V-11" /><title>DP-6B-143 · active leak · 88% probability</title></g>}<g className="map-alert amber" transform="translate(284 92)" onClick={() => setSelected("DP-03-018")}><circle r="10" /><circle r="4" /><path d="M0 -14 V-9" /></g>
      ...
      <div className="map-help">Drag to pan · scroll controls via buttons · click pipe or sensor</div><div className="demo-network-stamp"><span className="live-dot" /> DEMO NETWORK · Not live NCWSC infrastructure data</div>
    </div><div className="map-bottom-dock"><div className="pipe-inspector"><div className="dock-heading"><div><span className="eyebrow">NETWORK TRACE · {selected}</span><h3>{selectedPipe.status}</h3></div><button onClick={() => setShowPipeDetails((v) => !v)}>{showPipeDetails ? "Hide" : "Inspect"}</button></div>{showPipeDetails && <><div className="pipe-meta-grid"><span>Diameter <b>{selectedPipe.diameter}</b></span><span>Material <b>{selectedPipe.material}</b></span><span>Pressure <b>{selectedPipe.pressure}</b></span><span>Flow <b>{selectedPipe.flow}</b></span><span>Upstream <b>{selectedPipe.upstream}</b></span><span>Downstream <b>{selectedPipe.downstream}</b></span><span>Sensors <b>{selectedPipe.sensors}</b></span><span>AI risk <b>{selectedPipe.risk}</b></span></div><div className="trace-actions"><button onClick={() => toast(`Tracing upstream from ${selected}`)}><GitBranch size={12} /> Trace Upstream</button><button onClick={() => toast(`Tracing downstream from ${selected}`)}><GitBranch size={12} /> Trace Downstream</button><button onClick={() => toast(`Maintenance: ${selectedPipe.maintenance}`)}><Wrench size={12} /> Maintenance history</button></div></>}</div><div className="dma-inspector"><div className="dock-heading"><div><span className="eyebrow">DMA INTELLIGENCE · SIMULATED</span><h3>{selectedDMA} DMA <em className={`risk-${selectedDmaMetrics.risk.toLowerCase()}`}>{selectedDmaMetrics.risk}</em></h3></div><select value={selectedDMA} onChange={(event) => setSelectedDMA(event.target.value)}><option>Embakasi</option><option>CBD</option><option>Westlands</option></select></div><div className="dma-grid"><span>Inlet flow <b>{selectedDmaMetrics.inflow}</b></span><span>Consumption <b>{selectedDmaMetrics.consumption}</b></span><span>Minimum night flow <b>{selectedDmaMetrics.nightFlow}</b></span><span>NRW estimate <b>{selectedDmaMetrics.nrw}</b></span><span>Pressure <b>{selectedDmaMetrics.pressure}</b></span><span>Active alerts <b>{selectedDmaMetrics.alerts}</b></span><span>Water balance <b>{selectedDmaMetrics.balance}</b></span><span>AI risk score <b>{selectedDmaMetrics.risk}</b></span></div></div></div>
